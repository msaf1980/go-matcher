	...
	tests := []testGlob{
		{
			...
			want: &Glob{
				...
				Items: []items.NodeItem{
					...
					{Node: "a", Item: items.Byte('a')},
					{Node: "?", Item: items.Any(1)},
				},
			},
...
func Benchmark_Star_Byte_Miss_Regex(b *testing.B) {
	for i := 0; i < b.N; i++ {
...
func Benchmark_Star_Byte_Miss_Regex_Precompiled(b *testing.B) {
	w := tests.BuildGlobRegexp(globByte)
