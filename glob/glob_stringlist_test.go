...
func TestGlob_StringList(t *testing.T) {
	tests := []testGlob{
		...
		},
		{
			glob: "*.{bc,d}*.e",
			want: &Glob{
				Glob: "*.{bc,d}*.e", Node: "*.{bc,d}*.e",
				MinLen: 4, MaxLen: -1, Suffix: ".e",
				Items: []items.NodeItem{
					{Node: "*", Item: items.Star(0)},
					{Node: ".", Item: items.Byte('.')},
					{
						Node: "{bc,d}",
						Item: &items.StringList{
		...
		// glob star
		{
			...
			want: &Glob{
				...
				Items: []items.NodeItem{
					{Node: "*", Item: items.Star(0)},
					{Node: ".Cassandra.", Item: items.NewString(".Cassandra.")},
					{
						Node: "{BalanceCluster,BalanceStaging,Billing,UpProduction,UpTesting,WebCluster}",
						Item: &items.StringList{
							Vals: []string{
...
func Benchmark_Star_String_MissList_Regex_Precompiled(b *testing.B) {
